...
public interface StatelessServer: StatelessEntity {
    ...
    public suspend fun getBan(memberId: GenericId): ServerBan? =
        ServerBan.strategize(client, client.rest.member.getBan(memberId, id))

    public suspend fun getBans(): List<ServerBan> =
        client.rest.server.getServerBans(id).map { ServerBan.strategize(client, it) }
    ...
    public suspend fun createWebhook(builder: CreateWebhookRequestBuilder.() -> Unit): Webhook =
        DeckWebhook.strategize(client, client.rest.webhook.createWebhook(id, builder))

    public suspend fun getWebhook(webhookId: UUID): Webhook =
        DeckWebhook.strategize(client, client.rest.webhook.retrieveWebhook(webhookId, id))

    public suspend fun getWebhooks(channelId: UUID? = null): List<Webhook> =
        client.rest.webhook.retrieveServerWebhooks(id, channelId).map { DeckWebhook.strategize(client, it) }

    public suspend fun updateWebhook(webhookId: UUID, builder: UpdateWebhookRequestBuilder.() -> Unit): Webhook =
        DeckWebhook.strategize(client, client.rest.webhook.updateWebhook(webhookId, id, builder))
