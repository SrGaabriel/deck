...
import io.github.srgaabriel.deck.gateway.event.type.GatewayDocumentationReactionDeletedEvent
import java.util.*

/**
 * Called when someone removes a reaction from a [Documentation]
 * (not only when a reaction emote is removed completely from a doc)
 */
public data class DocumentationReactionRemoveEvent(
...
internal val EventService.documentationReactionRemove: EventMapper<GatewayDocumentationReactionDeletedEvent, DocumentationReactionRemoveEvent> get() = mapper { client, event ->
    DocumentationReactionRemoveEvent(
        client = client,
        barebones = event,
